...
func NewAccessoryResource(store store.Store, cache *lru.TwoQueueCache) *AccessoryResource {
...
func (cr *AccessoryResource) ByID(w http.ResponseWriter, r *http.Request) {
	...
	accessoryFromCache, ok := cr.cache.Get(id)
	if ok {
		render.JSON(w, r, accessoryFromCache)
		return
	}
