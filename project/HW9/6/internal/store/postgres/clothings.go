...
func (c ClothingsRepository) All(ctx context.Context, filter *models.ClothingsFilter) ([]*models.Clothing, error) {
	...
	if filter.Query != nil {
		...
		if err := c.conn.Select(&clothings, basicQuery, "%"+*filter.Query+"%"); err != nil {
			return nil, err
		}

		return clothings, nil
	}
	if err := c.conn.Select(&clothings, basicQuery); err != nil {
		return nil, err
	}
...
func (c ClothingsRepository) ByID(ctx context.Context, id int) (*models.Clothing, error) {
	...
	if err := c.conn.Get(clothing, "SELECT id, name, description, size, price, isAvailable FROM clothings WHERE id=$1", id); err != nil {
		return nil, err
	}
