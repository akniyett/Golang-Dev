...
func (u UsersRepository) Create(ctx context.Context, user *models.User) error {
	_, err := u.conn.ExecContext(ctx, "INSERT INTO users(nick, password, bio, email) VALUES ($1, $2, %3, $4)", user.Nick, user.Password, user.Bio, user.Email)
	if err != nil {
		return err
	}
	return nil
...
func (u UsersRepository) ByID(ctx context.Context, id int) (*models.User, error) {
	...
	if err := u.conn.GetContext(ctx, user, "SELECT id, nick, password, bio, email FROM users WHERE id=$1", id); err != nil {
		return nil, err
	}
...
func (u UsersRepository) Update(ctx context.Context, user *models.User) error {
	_, err := u.conn.ExecContext(ctx, "UPDATE users SET nick = $1, password = $2, bio = $3, email = $4 WHERE id = $5", user.Nick, user.Password, user.Bio, user.Email, user.ID)
	if err != nil {
		return err
	}
	return nil
...
func (u UsersRepository) Delete(ctx context.Context, id int) error {
	_, err := u.conn.ExecContext(ctx, "DELETE FROM users WHERE id = $1", id)
	if err != nil {
		return err
	}
	return nil
